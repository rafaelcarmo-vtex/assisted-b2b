...
const isOrderBuilderRoute = () => window.location.hash === '#order-builder';
...
const OrderBuilderPage: React.FC = () => {
  ...
  const [filledSlices, setFilledSlices] = useState<FilledSlices>(EMPTY_SLICES);
            ...
            <section key={canvasStateKey} className={`w-full flex flex-col gap-[60px] canvas-page-enter ${isCanvasUpdating ? 'pointer-events-none' : ''}`}>
              <section className="w-full flex flex-col gap-[60px]">
                ...
                {isQuotePending && (
                  <div
                    role="status"
                    className={`quote-status-banner w-full h-[52px] -mt-[20px] -mb-[20px] rounded-full px-5 flex items-center gap-2 ${
                      isQuoteRevised ? 'bg-[#01663F]' : 'bg-[#1F1F1F]'
                    }`}
                  >
                    <span
                    ...
                    <span className="text-[14px] leading-5 tracking-[-0.01em] font-medium text-white truncate">
                      {isQuoteRevised
...
const App: React.FC = () => {
  ...
  useEffect(() => {
    const onHashChange = () => setShowOrderBuilder(isOrderBuilderRoute());
