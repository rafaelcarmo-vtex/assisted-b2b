...
export default async function handler(req: Request): Promise<Response> {
  ...
  if (!process.env.GROQ_API_KEY) {
    return new Response(
      JSON.stringify({
        error:
          'GROQ_API_KEY is not set. Get a free key at https://console.groq.com and add it to .env.local as GROQ_API_KEY=...',
      }),
  ...
  try {
    payload = (await req.json()) as { messages: UIMessage[] };
  } catch {
  ...
  const result = streamText({
    ...
    stopWhen: [stepCountIs(8), hasToolCall('requestQuote')],
    temperature: 0.4,
  });
