...
const QUOTE_SALES_REP_NAME = 'Amanda Brown';
...
const isOrderBuilderRoute = () => window.location.hash.startsWith('#order-builder');
const isQuotesRoute = () => window.location.hash === '#quotes';

const getOrderBuilderHashParams = (): URLSearchParams => {
...
const readInitialPendingQuoteFromHash = (): boolean => {
  if (typeof window === 'undefined') return false;
  if (!isOrderBuilderRoute()) return false;
  const params = getOrderBuilderHashParams();
  return params.get('state') === 'pending-quote' || !!params.get('quote');
};
...
const OrderBuilderPage: React.FC = () => {
  ...
  const initialPendingQuoteFromHash = useMemo(() => readInitialPendingQuoteFromHash(), []);
  ...
  const [filledSlices, setFilledSlices] = useState<FilledSlices>(() =>
    initialPendingQuoteFromHash ? FULL_SLICES : EMPTY_SLICES
  );
  ...
  const [isQuotePending, setIsQuotePending] = useState(initialPendingQuoteFromHash);
  ...
  const getProductDiscountPercent = (item: ProductLineItem) =>
    isQuoteRevised ? REVISED_PRODUCT_DISCOUNTS[item.id] ?? 0 : 0;

  const getEffectiveUnitPriceValue = (item: ProductLineItem) => {
    const base = parseCurrencyToNumber(item.unitPrice);
    return base * (1 - getProductDiscountPercent(item));
  };
  ...
  const initialAssistantMessage = useMemo<UIMessage>(
    () => ({
      id: 'greeting',
      role: 'assistant',
      parts: [{ type: 'text', text: GREETING_MESSAGE, state: 'done' }],
    }),
    []
  );
            ...
            <div className="pb-10 space-y-10">
              {messages.map((chatMessage) => {
                ...
                if (chatMessage.role === 'assistant') {
                  ...
                  const isLatestAssistantStreaming = isCanvasUpdating && isLastMessage;
                  ...
                  return (
                    <div key={chatMessage.id} className="chat-message-appear w-full flex flex-col gap-4">
                      ...
                      {isRevisedSyntheticResponse && quoteCard && (
                        <div className="text-[14px] leading-[1.4] tracking-[-0.01em] text-[#1F1F1F] flex flex-col gap-3">
                          <p>
                ...
                const displayUserText = textContent
                  .replace(/\n*\[(?:User attached a file|The user wants to request a quote|Request-quote context|Quote-flow continuation|Quote-request STEP \d|Order is in (?:quote-pending|draft) state)[^\]]*\]\n*/g, '')
                  .trim();
              ...
              {(chatStatus === 'submitted' || isCanvasUpdating || isSubmittingQuote) && (
      ...
      >
        <main ref={canvasScrollRef} className={`panel-scroll flex-1 bg-white px-[60px] pt-[60px] overflow-y-auto ${isBulkImporting ? 'pb-[40px]' : 'pb-[60px]'}`}>
          {isBulkImporting ? (
            ...
            <section key={canvasStateKey} className={`w-full flex flex-col gap-[60px] canvas-page-enter ${isCanvasUpdating ? 'pointer-events-none' : ''}`}>
              <section className="w-full flex flex-col gap-[60px]">
                ...
                {isQuotePending && (
                  isQuoteRevised ? (
                    <div
                      role="status"
                      className="quote-status-banner w-full h-[52px] -mt-[20px] -mb-[20px] rounded-[8px] px-5 flex items-center gap-3 bg-[#E9FCE3]"
                    >
                      <span
                        className="material-symbols-outlined text-[20px] text-[#01663F] shrink-0"
                        style={{ fontVariationSettings: "'FILL' 1, 'wght' 400, 'GRAD' 0, 'opsz' 20" }}
                        aria-hidden="true"
                      >
                        check_circle
                      ...
                      <span className="text-[14px] leading-5 tracking-[-0.01em] text-[#1F1F1F] truncate">
                        <span className="font-semibold">Quote ready for checkout.</span> Review the updated terms and go to checkout.
                      </span>
                    </div>
                  ) : (
                    <div
                      role="status"
                      className="quote-status-banner w-full h-[52px] -mt-[20px] -mb-[20px] rounded-[8px] px-5 flex items-center gap-3 bg-[#FFF1DC]"
                    >
                      <span
                        className="material-symbols-outlined text-[20px] text-[#E29911] shrink-0"
                      ...
                      <span className="text-[14px] leading-5 tracking-[-0.01em] text-[#1F1F1F] truncate">
                        <span className="font-semibold">Quote request submitted.</span> It is pending review.
                      </span>
                    </div>
                  )
                )}
                  ...
                  <div className="w-full flex-1">
                    <div className="flex flex-col">
                      <div className={`${ORDER_ROW_HEADER_CLASS} border-b border-[#EBEBEB] flex items-center text-[14px] leading-5 tracking-[-0.01em] text-[#707070]`}>
                        ...
                        <span className="w-[100px] text-right">Quantity</span>
                        {isQuoteRevised && <span className="w-[100px] text-right">Discount</span>}
                        <span className="w-[100px] text-right">Unit price</span>
                      ...
                      {displayedProducts.map((item, index) => (
                        ...
                        >
                          <div className="flex items-center">
                            ...
                            {isQuoteRevised && (
                              <div className="w-[100px] text-right text-[14px] leading-5 tracking-[-0.01em] font-medium text-[#01663F]">
                                {getProductDiscountPercent(item) > 0
                                  ? `${Math.round(getProductDiscountPercent(item) * 100)}%`
                                  : ''}
                              </div>
                            )}
                            <div className="w-[100px] text-right text-[14px] leading-5 tracking-[-0.01em] text-[#1F1F1F]">
                              {getProductDiscountPercent(item) > 0 ? (
              ...
              <section className="w-full flex flex-col gap-5">
                ...
                <div className="flex flex-col">
                  ...
                  {isQuoteRevised && productsDiscountValue > 0 && (
                    <div className={`${ORDER_ROW_HEIGHT_CLASS} flex items-center border-b border-[#EBEBEB]`}>
...
const App: React.FC = () => {
  const [showOrderBuilder, setShowOrderBuilder] = useState<boolean>(() => isOrderBuilderRoute());
  const [showQuotes, setShowQuotes] = useState<boolean>(() => isQuotesRoute());

  useEffect(() => {
    const onHashChange = () => {
      setShowOrderBuilder(isOrderBuilderRoute());
      setShowQuotes(isQuotesRoute());
    };
    ...
    return (
      <div className="h-screen bg-white">
        <QuotesPage
          contracts={CONTRACTS}
          defaultContractId="1"
          orgUnitName="Operations and facilities"
          onManageOrg={() => {}}
        />
      </div>
    );
